...
import { ActaEntregaCreate, ActaEntregaUpdate } from '../models/actaEntrega.model';

const ACTA_ENTREGA_SELECT = `
  id, numero_acta, fecha_ingreso, fecha_entrega, hora_entrega, status, created_at, updated_at,
...
interface BorradorEntregaInput {
...
export async function guardarBorradorEntrega(formData: BorradorEntregaInput) {
  ...
  const patente = patenteRaw || `TMP${Date.now().toString(36).slice(-7).toUpperCase()}`;
  ...
  if (formData.acta_entrega_id) {
    const { data, error } = await supabase
      .from('actas_entrega')
      .update(patch)
      .eq('id', formData.acta_entrega_id)
      .select()
      .single();
    if (error) throw error;
    return { acta_entrega: data, cliente, vehiculo, reutilizado: true };
  }
  ...
  const { data: acta_entrega, error } = await supabase
    .from('actas_entrega')
    .insert({
      vehiculo_id: vehiculo.id,
      cliente_id: cliente.id,
  ...
  if (error) throw error;
  return { acta_entrega, cliente, vehiculo, reutilizado: false };
}

export async function crearActaEntrega(datos: ActaEntregaCreate) {
  if (!datos.vehiculo_id) throw new Error('No se puede crear un acta de entrega sin vehículo asociado.');
  const { data, error } = await supabase.from('actas_entrega').insert(datos).select().single();
  if (error) throw error;
  return data;
}

export async function actualizarActaEntrega(id: string, datos: ActaEntregaUpdate) {
  const { data, error } = await supabase
    .from('actas_entrega')
...
export async function listarActasEntrega({
  status,
  limite = 30,
}: { status?: string; limite?: number } = {}) {
  let query = supabase
    .from('actas_entrega')
    .select(ACTA_ENTREGA_SELECT)
...
export async function cargarActaEntregaCompleta(actaEntregaId: string) {
  const { data, error } = await supabase
    .from('actas_entrega')
...
export async function buscarBorradorEntregaPorPatente(patente: string) {
...
export async function buscarActasEntregaPorPatente(
  patente: string,
...
export async function eliminarActaEntrega(id: string) {
  const { data, error } = await supabase
    .from('actas_entrega')
    .delete()
    .eq('id', id)
